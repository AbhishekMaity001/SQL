...
--49
SELECT NAME,DOB, EXTRACT(DAY FROM (date_trunc('month', DOB) + interval '1 month' - interval '1 day')::date )
AS end_of_month, EXTRACT(DAY FROM DOB) AS ACTUAL FROM PROGMR
WHERE EXTRACT(DAY FROM (date_trunc('month', DOB) + interval '1 month' - interval '1 day')::date )=EXTRACT(DAY FROM DOB)
;

--50
SELECT SCOST, DCOST, SCOST-DCOST AS DIFFERENCE_COST FROM SOFTWARE ORDER BY DIFFERENCE_COST DESC

--51
SELECT TITLE FROM SOFTWARE
WHERE SPLIT_PART(TITLE, ' ', 2) NOT IN  ('')

--52
SELECT NAME, DOB, DOJ FROM PROGMR
WHERE EXTRACT(MONTH FROM DOB) = EXTRACT(MONTH FROM DOJ)

--53
SELECT DEV_IN, COUNT(TITLE) AS PACKG_DEV_IN_EACH_LANG FROM SOFTWARE
GROUP BY DEV_IN
...
SELECT S.RETAILER_CODE, SUM(TAX1_AMT+TAX2_AMT)
FROM STORE AS S , SALES AS SAL
WHERE S.RETAILER_CODE = SAL.RETAILER_CODE
GROUP BY S.RETAILER_CODE
